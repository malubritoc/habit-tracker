...
type patchHabitFormInputs = z.infer<typeof patchHabitFormSchema>;

export function PatchHabitForm({
  record,
  setOpen,
}: {
  record: Record;
  ...
  async function fetchHabit() {
    try {
      ...
      toast({
        variant: "default",
        title: "Erro ao carregar dados do hábito",
      });
    }
  }
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<patchHabitFormInputs>({
    resolver: zodResolver(patchHabitFormSchema),
    mode: "onBlur",
  });
  ...
  async function handlePatchNewHabit(data: patchHabitFormInputs) {
    setLoading(true);
    try {
      // console.log(data);
      ...
      toast({
        variant: "success",
        title: "Hábito editado com sucesso",
        description: "Seu hábito foi editado com sucesso.",
      });
    } catch (error) {
      console.log(error);
      toast({
        variant: "destructive",
        title: "Erro ao editar hábito",
        description: "Ocorreu um erro ao editar o hábito, tente novamente.",
      });
      setLoading(false);
    }
  }
  ...
  return (
    <form
      ...
      onSubmit={handleSubmit(handlePatchNewHabit)}
    >
